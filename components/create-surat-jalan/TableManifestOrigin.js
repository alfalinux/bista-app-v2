...
const TableManifestOrigin = ({ dataManifestOrigin, initValues, setInitValues }) => {
  const [checkedManifest, setCheckedManifest] = useState([]);
  const [checkedAll, setCheckedAll] = useState(false);
  const checkbox = document.querySelectorAll("#checkboxorigin");
    ...
    if (checkedAll) {
      ...
      setInitValues((prevState) => ({
        ...
        checkedManifest: prevState.checkedManifest.filter(
          (d) => !dataManifestOrigin.map((d) => d.noManifest).includes(d.noManifest)
        ),
      }));
    } else {
      setCheckedAll(true);
      setCheckedManifest(dataManifestOrigin);
      for (let item of checkbox) {
        item.checked = true;
      }
      setInitValues((prevState) => ({
        ...prevState,
        checkedManifest: [...prevState.checkedManifest, ...dataManifestOrigin],
      }));
    }
  };
  ...
  useEffect(() => {
    if (initValues.checkedManifest.length === 0) {
      ...
      setCheckedAll(false);
    } else if (checkedManifest.length === dataManifestOrigin.length) {
      setCheckedAll(true);
    } else {
      setCheckedAll(false);
    }
  ...
  return (
    <div className="w-full">
      <h2 className="font-semibold text-gray-800 text-sm dark:text-gray-100">Manifest Origin</h2>
      {dataManifestOrigin.length > 0 ? (
        <table className="w-full table-auto shadow-md">
          ...
          <tbody>
            {dataManifestOrigin.map((data, index) => (
              <tr key={index}>
                <td
...
export default TableManifestOrigin;
