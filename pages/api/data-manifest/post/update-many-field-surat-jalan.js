...
const handler = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ status: res.statusCode, message: "Gagal terhubung ke database manifest" });
    client.close();
    return;
  }
  ...
  if (req.method === "PATCH") {
    let result;
    try {
      result = await pushManySuratJalan(client, "data-manifest", filter, update);
    } catch (error) {
